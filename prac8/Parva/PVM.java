...
class PVM {
  ...
  static void postMortem(OutFile results, int pcNow) {
    ...
    switch (ps) {
      ...
      case nullRef:
        ...
        break;
      case rootNeg:
        results.write("Negative square root");
        break;
      default:
      ...
      switch (cpu.ir) { // execute
        ...
        case PVM.rem: // integer division (remainder)
          ...
          if (tos == 0)
            ...
            push(pop() % tos);
          break;
        case PVM.sqr: // square integer
          tos = pop();
          push(tos * tos);
          break;
        case PVM.sqrt: // square root integer
          tos = pop();
          if (tos < 0)
            ps = rootNeg;
          else
  ...
  public static int opCode(String str) {
    ...
    // Simple linear search. A hashtable or dictionary might be a useful
    // improvement!
    int op = 0;
